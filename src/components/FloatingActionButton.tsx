...
export default function FloatingActionButton({ setActiveTab }: Props) {
  ...
  return (
    <div ref={fabRef} className="fixed bottom-8 right-8 z-50 flex flex-col-reverse items-end gap-3 group/fab">
      {/* Main Button */}
      <button
        onClick={() => setIsFabOpen(!isFabOpen)}
        className="relative w-14 h-14 bg-gradient-to-tr from-sky-400 to-blue-500 text-white rounded-[20px] shadow-[0_8px_32px_rgba(56,189,248,0.4)] flex items-center justify-center transition-all duration-300 hover:scale-105 hover:shadow-[0_12px_48px_rgba(56,189,248,0.5)] p-0 z-10 before:absolute before:inset-0 before:rounded-[20px] before:border before:border-white/40 before:bg-white/10 before:backdrop-blur-md outline-none focus:outline-none focus:ring-4 focus:ring-sky-300/30"
      >
