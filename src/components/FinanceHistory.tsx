        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.keys(groupedHistory).sort().reverse().map(month => (
            <div key={month} className="space-y-3">
              <h3 className="text-sm font-bold opacity-60 border-b border-sky-300/30 pb-1">{month}</h3>
              {groupedHistory[month].map(record => {
                ...
                return (
                  <div 
                    ...
                    className="p-4 glass-panel rounded-xl border border-sky-300/30 hover:bg-sky-50/40 cursor-pointer transition-all group flex items-center justify-between"
                  >
                    <div>
                    ...
                    <ChevronRight className="w-5 h-5 text-sky-700/80 group-hover:text-cyan-600 transform group-hover:translate-x-1 transition-all" />
                  </div>
                );
              })}
            </div>
