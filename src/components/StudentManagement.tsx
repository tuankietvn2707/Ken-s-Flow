...
export default function StudentManagement({ students, addStudent, updateStudent, deleteStudent, classes, markClassesAsPaid }: Props) {
  ...
  const renderClassCard = (cls: ClassSession, idx: number, isPaid: boolean) => (
    <div key={cls.id} className="glass-panel rounded-3xl border border-sky-300/30 shadow-[0_10px_30px_rgba(0,0,0,0.04)] overflow-hidden">
      <div className={`${isPaid ? 'bg-emerald-50/50' : 'bg-blue-50/50'} px-6 py-3 border-b border-sky-300/40 flex justify-between items-center`}>
        ...
        <div className="text-sm font-medium text-sky-700/80 glass-panel px-2 py-1 rounded border border-sky-300/40">
          {cls.duration} buổi
        </div>
      </div>
      <div className="p-6">
    ...
    <Draggable key={student.id} draggableId={student.id} index={index}>
      {(provided, snapshot) => (
        <tr 
          ...
          className={`hover:bg-sky-50/50 transition-colors ${snapshot.isDragging ? 'bg-sky-50/50 shadow-md' : ''} ${isInactive ? 'opacity-60 bg-sky-50/40' : ''}`}
        >
          <td className="px-6 py-4 whitespace-nowrap text-sky-600/50">
  ...
  return (
    <div className="space-y-6">
      ...
      {isFormOpen && (
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-8" noValidate>
            ...
            <fieldset className="bg-sky-50/40 p-4 rounded-2xl border border-sky-200/40">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mt-3">
                <div>
                  ...
                  <input
                    ...
                    className={`mt-1 block w-full rounded-xl shadow-sm sm:text-sm p-2 border bg-white focus:ring-sky-500 ${fieldErrors.lastName ? 'border-rose-500 focus:border-rose-500' : 'border-sky-300/50 focus:border-sky-500'}`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`mt-1 block w-full rounded-xl shadow-sm sm:text-sm p-2 border bg-white focus:ring-sky-500 ${fieldErrors.firstName ? 'border-rose-500 focus:border-rose-500' : 'border-sky-300/50 focus:border-sky-500'}`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`mt-1 block w-full rounded-xl shadow-sm sm:text-sm p-2 border bg-white focus:ring-sky-500 ${fieldErrors.birthYear ? 'border-rose-500 focus:border-rose-500' : 'border-sky-300/50 focus:border-sky-500'}`}
                  />
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mt-3">
                <div>
                  ...
                  <select
                    ...
                    className="mt-1 block w-full rounded-xl border-sky-300/50 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm p-2 border glass-panel"
                  >
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mt-3">
                <div>
                  ...
                  <div className="mt-1 relative rounded-xl shadow-sm">
                    <input
                      ...
                      className={`block w-full rounded-xl shadow-sm sm:text-sm p-2 border bg-white pr-12 focus:ring-amber-500 ${fieldErrors.fee ? 'border-rose-500 focus:border-rose-500' : 'border-sky-300/50 focus:border-amber-500'}`}
                    />
                ...
                <div>
                  ...
                  <select
                    ...
                    className="mt-1 block w-full rounded-xl border-sky-300/50 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm p-2 border glass-panel"
                  >
            ...
            <div className="flex justify-end pt-4 border-t border-sky-300/40">
              <button
                type="button"
                onClick={closeForm}
      ...
      <motion.div 
        ...
        className="glass rounded-3xl shadow-[0_8px_32px_rgba(14,165,233,0.08)] border-sky-300/30 overflow-hidden mb-8"
      >
        <div className="px-6 py-4 border-b border-sky-300/20 bg-sky-50/40">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-sky-300/40">
            <thead className="bg-sky-50/50 border-b border-sky-300/20">
              <tr>
      ...
      {inactiveStudents.length > 0 && (
        <motion.div 
          ...
          className="glass rounded-3xl shadow-[0_8px_32px_rgba(14,165,233,0.08)] border-sky-300/30 overflow-hidden opacity-80"
        >
          <div className="px-6 py-4 border-b border-sky-300/30 bg-sky-100/50">
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            {/* Background overlay */}
            <div className="fixed inset-0 bg-sky-900/20 backdrop-blur-sm transition-opacity" aria-hidden="true" onClick={() => setSelectedStudent(null)}></div>
            ...
            <div className="relative z-10 inline-flex flex-col align-bottom glass rounded-[32px] text-left overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.08)] transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full max-h-[90vh]">
              <div className="bg-white px-6 py-4 border-b border-sky-300/40 flex justify-between items-center shrink-0">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="text-sm font-medium text-sky-600 hover:text-indigo-800 bg-sky-50 hover:bg-indigo-100 px-3 py-1.5 rounded-xl transition-colors"
                  >
                    Sửa hồ sơ
                  </button>
                  <button 
                    onClick={() => setSelectedStudent(null)}
                    className="text-sky-600/50 hover:text-sky-700/80 bg-sky-100/50 hover:bg-sky-200 rounded-full p-1.5 transition-colors"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                    ...
                    <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
                      <div>
                        ...
                        <p className="font-semibold text-sky-950">{selectedStudent.name}</p>
                      </div>
                      <div>
                        <p className="text-sm text-sky-700/80 mb-1">Năm sinh</p>
                        <p className="font-semibold text-sky-950">{selectedStudent.birthYear || '—'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-sky-700/80 mb-1">Trình độ hiện tại</p>
                        <p className="font-semibold text-sky-950">{selectedStudent.currentLevel || '—'}</p>
                      </div>
                      <div>
                      ...
                      <div className="md:col-span-4">
                        ...
                        <p className="font-semibold text-sky-950">{selectedStudent.schedule || '—'}</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    {(() => {
                      ...
                      if (unpaidClasses.length === 0) {
                        return (
                          <div className="text-center py-10 glass-panel rounded-3xl border border-sky-300/40 border-dashed">
                      ...
                      return Object.entries(groupedClasses).map(([monthYear, monthClasses]) => (
                        <div key={monthYear} className="glass-panel rounded-3xl border border-sky-300/30 shadow-[0_10px_30px_rgba(0,0,0,0.04)] overflow-hidden">
                          <button
                            ...
                            className="w-full px-6 py-4 flex justify-between items-center bg-sky-50/40 hover:bg-sky-100/50 transition-colors"
                          >
                          ...
                          {expandedMonths[monthYear] && (
                            <div className="p-4 space-y-4 bg-sky-50/40">
                ...
                <div className="glass-panel rounded-3xl border border-sky-300/30 shadow-[0_10px_30px_rgba(0,0,0,0.04)] p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="flex items-baseline gap-3">
                      <p className="text-2xl font-bold text-sky-950">{formatVND(getUnpaidAmount(selectedStudent.id))}</p>
